...
import (
	...
	"github.com/chayimamaral/vecx/backend/internal/httpapi/handlers"
...
func NewRouter(cfg config.Config, pool *pgxpool.Pool, auditPool *pgxpool.Pool, staticFS fs.FS) http.Handler {
	...
	r.Use(apiMiddleware.RequestLoggerFiltered)
	...
	serproServicoEnquadramentoHandler := handlers.NewSerproServicoEnquadramentoHandler(serproServicoEnquadramentoService)
	...
	r.NotFound(func(w http.ResponseWriter, r *http.Request) {
		...
		ServeSPA(staticFS).ServeHTTP(w, r)
	})
...
func registerRoutes(
	...
	serproServicoEnquadramentoHandler *handlers.SerproServicoEnquadramentoHandler,
	...
	r.With(requireAuth).Get("/salarios-minimos", salarioMinimoHandler.List)
