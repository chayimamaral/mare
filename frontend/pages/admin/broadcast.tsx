...
export default function BroadcastSessionsPage() {
  useRouteClientGuard();
  const toast = useRef<Toast>(null);

  const { data: userRole = null } = useQuery<string | null>({
    queryKey: ['broadcast-page-role'],
    queryFn: async () => {
  ...
  return (
    <div className="grid">
      <div className="col-12">
        <Card title="Broadcast & Sessões">
          <Toast ref={toast} />

          <Message
            severity="info"
            text="Broadcast em massa permanece em evolução. Abaixo, monitoramento de sessões ativas."
            className="mb-3"
          />
          ...
          <div className="flex justify-content-end mb-3">
            <Button
              ...
              onClick={() => {
                void refetch();
                toast.current?.show({ severity: 'info', summary: 'Sessões', detail: 'Atualizando…', life: 1500 });
              }}
              loading={isFetching}
              disabled={!isSuper}
            />
