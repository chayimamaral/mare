...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/" name="Evento_EPEC">

    <xsl:variable name="prot" select="n:retEvento/n:infEvento/n:nProt" />
    <div id="EPEC{$prot}" class="GeralXslt">
      <fieldset>
        <legend class="titulo-aba">EPEC-Emissão em Contingência</legend>
        <table class="box">
          <tr>
        ...
        <table class="box">
          <tr class="col-2">
            <td colspan="2">
              ...
              <span>
                <xsl:value-of select="n:evento/n:infEvento/n:tpEvento"/> - EPEC-Emissão em Contingência
              </span>
            </td>
            <td colspan="2">
        ...
        <table class="box">
          <tr class="col-3">
            ...
            <td>
              <label>Versão</label>
              <span>
                <xsl:value-of select="n:evento/n:infEvento/n:detEvento/@versao"/>
              </span>
            </td>
            <td>
              <label>Orgão Autor</label>
              <span>
                <xsl:value-of select="n:evento/n:infEvento/n:detEvento/n:cOrgaoAutor"/> -
                <xsl:call-template name="nomeUF">
                  <xsl:with-param select="n:evento/n:infEvento/n:detEvento/n:cOrgaoAutor" name="uf">
                  </xsl:with-param>
                </xsl:call-template>                
              </span>
            </td>
          ...
          <tr class="col-3">
            ...
            <td>
              ...
              <span>
                <xsl:call-template name="formatDateTimeFuso">
                  <xsl:with-param name="dateTime" select="n:evento/n:infEvento/n:detEvento/n:dhEmi"/>
                  <xsl:with-param name="include_as" select="1"/>
                </xsl:call-template>                
              </span>
            </td>
          ...
          <tr class="col-3">
            <td>
              ...
              <span>
                <xsl:variable name="tpNotaFiscal" select="n:evento/n:infEvento/n:detEvento/n:tpNF"/>
                <xsl:choose>
                  <xsl:when test="$tpNotaFiscal = 0">
                    0 - Entrada
                  </xsl:when>
                  <xsl:when test="$tpNotaFiscal = 1">
                    1 - Saída
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:value-of select="$tpNotaFiscal"/> 
                  </xsl:otherwise>
                </xsl:choose>
              </span>
            </td>
            <td>
              ...
              <span>
                <xsl:value-of select="n:evento/n:infEvento/n:detEvento/n:IE"/>
              </span>
            </td> 
          </tr>  
        </table>
        ...
        <table class="box">
          <tr class="col-3">
            <td>
              <label>UF</label>
              <span>
                <xsl:value-of select="n:evento/n:infEvento/n:detEvento/n:dest/n:UF"/>
              </span>
            </td>
            <td>
              <xsl:variable name="cnpj" select="n:evento/n:infEvento/n:detEvento/n:dest/n:CNPJ"/>
              <xsl:variable name="cpf" select="n:evento/n:infEvento/n:detEvento/n:dest/n:CPF"/>
              <xsl:variable name="idestrang" select="n:evento/n:infEvento/n:detEvento/n:dest/n:idEstrangeiro"/>
            ...
            <td>
              <label>IE</label>
              <span>
                <xsl:value-of select="n:evento/n:infEvento/n:detEvento/n:dest/n:IE"/>
              </span>
            </td>
          </tr>
            ...
            <td>
              ...
              <span>
                <xsl:call-template name="format2Casas">
                  <xsl:with-param name="num" select="n:evento/n:infEvento/n:detEvento/n:dest/n:vNF"/>
                </xsl:call-template>
              </span>
            </td>
            <td>
              ...
              <span>
                <xsl:call-template name="format2Casas">
                  <xsl:with-param name="num" select="n:evento/n:infEvento/n:detEvento/n:dest/n:vICMS"/>
                </xsl:call-template>
              </span>
            </td>
            <td>
              <label>Valor total ICMS ST</label>
              <span>
                <xsl:call-template name="format2Casas">
                  <xsl:with-param name="num" select="n:evento/n:infEvento/n:detEvento/n:dest/n:vST"/>
                </xsl:call-template>
              </span>
            </td>
          </tr>
