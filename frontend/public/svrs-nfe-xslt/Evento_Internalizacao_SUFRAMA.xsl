...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/" name="Evento_Internalizacao_SUFRAMA">

    <xsl:variable name="prot" select="n:retEvento/n:infEvento/n:nProt" />
    <div id="EventoInternalizacaoSUFRAMA{$prot}" class="GeralXslt">
      <fieldset>
        <legend class="titulo-aba">Internalização SUFRAMA</legend>
        <table class="box">
          <tr>
        ...
        <table class="box">
          <tr class="col-2">
            <td colspan="2">
              ...
              <span>
                <xsl:value-of select="n:evento/n:infEvento/n:tpEvento"/> - Internalização SUFRAMA
              </span>
            </td>
            <td colspan="2">
