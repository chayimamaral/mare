...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/" name="Evento_Nao_Realizado">

    <xsl:variable name="prot" select="n:retEvento/n:infEvento/n:nProt" />
    <div id="OperacaoNaoRealizada{$prot}" class="GeralXslt">
      <fieldset>
        <legend class="titulo-aba">Operação não Realizada</legend>
        <table class="box">
          <tr>
        ...
        <table class="box">
          <tr class="col-2">
            <td colspan="2">
              ...
              <span>
                <xsl:value-of select="n:evento/n:infEvento/n:tpEvento"/> - Operação não Realizada
              </span>
            </td>
            <td colspan="2">
